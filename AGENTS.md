...
- Keep architecture: auth / endpoint / executor / translate / api, plus accepted `internal/store` (SQLite), `internal/control` (console facade), and `internal/runtime` (Manager lifecycle). The rest of the proposed split in `docs/REFACTORING.md` is not live until the matching cross-package stage is accepted. Account types and Pool stay in `accounts`; SQLite lives in `internal/store`; process tables live in runtime. `accounts` must not import `runtime`.
