...
- Keep architecture: auth / endpoint / executor / translate / api, plus accepted `internal/store` (SQLite), `internal/control` (console facade), `internal/runtime` (Manager lifecycle), and `internal/providers/qoder` (Qoder HOME/CLI/worker protocol). The rest of the proposed split in `docs/REFACTORING.md` is not live until the matching cross-package stage is accepted. Account types and Pool stay in `accounts`; SQLite lives in `internal/store`; process tables live in runtime. Qoder HOME/CLI and worker HTTP live in `providers/qoder`; runtime/api/executor call those methods but HTTP handlers stay in api. `accounts` must not import `runtime`. Provider packages must not receive `http.ResponseWriter`.
