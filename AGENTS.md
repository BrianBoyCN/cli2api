...
- Keep architecture: auth / endpoint / executor / translate / api, plus accepted `internal/store` (SQLite) and `internal/control` (thin console facade over Manager). The rest of the proposed split in `docs/REFACTORING.md` is not live until the matching cross-package stage is accepted. Account types stay in `accounts`; SQLite lives in `internal/store`; control must not reimplement process start/stop.
