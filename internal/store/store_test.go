...
func TestStoreCreatesNamedAPIKeys(t *testing.T) {
	...
	if _, err := store.GetAPIKey(ctx, created.ID); !errors.Is(err, accounts.ErrAPIKeyNotFound) {
...
func TestStoreSavesNativeCredential(t *testing.T) {
	...
	defer store.Close()
	account, err := store.Create(ctx, accounts.CreateAccount{Name: "Imported", Enabled: true})
	if err != nil {
		t.Fatal(err)
	}
	want := accounts.NativeCredential{UserBlob: []byte("encrypted-user"), MachineID: "machine-1"}
...
func TestStoreListsUpdatesAndDeletesAccounts(t *testing.T) {
	...
	defer store.Close()
	first, _ := store.Create(ctx, accounts.CreateAccount{Name: "A", Enabled: true})
	_, _ = store.Create(ctx, accounts.CreateAccount{Name: "B", Enabled: false})
...
func TestCooldownsSurviveReopen(t *testing.T) {
	...
	account, err := store.Create(ctx, accounts.CreateAccount{Name: "Work", Enabled: true, MaxInFlight: 4})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveCooldownsReplacesStaleRows(t *testing.T) {
	...
	account, err := store.Create(ctx, accounts.CreateAccount{Name: "Work", Enabled: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStorePersistsQuotaAndStatus(t *testing.T) {
	...
	defer store.Close()
	account, err := store.Create(ctx, accounts.CreateAccount{Name: "Quota", Enabled: true})
	if err != nil {
		t.Fatal(err)
	}
